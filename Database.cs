...
namespace Expire
{
	...
	public class Database:IDisposable
	{
		...
		static string			 db =Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "db.dat");
		...
		int years = int.Parse(System.DateTime.Now.ToString("yyyy",gro));
		int months =Convert.ToInt32(DateTime.Now.ToString("MM",gro));
//		//int day =Convert.ToInt32(DateTime.Now.ToString("dd",gro));
//		DateTime dTest = new DateTime(2014,4,1);
//
//		
//		int years =dTest.Year;
//		int months =dTest.Month;
		...
		if(checkMonth>12){
			...
			strYear= (years+1).ToString();
				strMonth=(checkMonth-x).ToString();
		}
		else{
		
		strYear= (years).ToString();
		strMonth=(checkMonth+3).ToString();
		}
		...
		public void getAll(int textType,int orderBy=0){
			...
			if(textType ==4){
				query= String.Format("select * from item where expdate<'{0}' order by {1} ",GetDateNow(),orderBBy);
			}
			if(textType ==5){
				query= String.Format("select * from item where expdate >'{0}' and expdate < '{1}' order by {2} ",GetDateNow(),getNearExp(1,11),orderBBy);
			}
			
			if(textType ==6){
				
				query= String.Format("select * from item where expdate>'{0}' and expdate < '{1}' order by {2} ",getNearExp(1,11),getNearExp(4,8),orderBBy);
			}
			if(textType ==7){
